...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `City-scoped documents — ${countrySlug}` }
}

export default async function CityScopedDocumentsPage({ params, searchParams }: Props) {
  ...
  const result = await adminFetch<DocumentTypeListResult>(
    `/admin/v1/document-types?countryId=${country.id}&status=ACTIVE&scope=CITY&page=${page}&pageSize=${PAGE_SIZE}`,
  ...
  return (
    <div className="page-content animate-slide-up">
      ...
      <div className="admin-card flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight text-foreground">City-Scoped Documents</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">Required once per vendor per city in {country.name} — inherited by that vendor's outlets there.</p>
        </div>
      </div>
      ...
      <ActiveDocumentsTable
        ...
        basePath={`/countries/${country.slug}/documents/city`}
        canWrite={canWrite}
        emptyTitle="No city-scoped documents"
        emptyDescription="Documents tied to a specific city will show up here."
      />
    </div>
  )
}
