...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `Outlet-scoped documents — ${countrySlug}` }
}
  ...
  return (
    <div className="page-content animate-slide-up">
      ...
      <div className="admin-card flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight text-foreground">Outlet-Scoped Documents</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">Required separately for each outlet in {country.name}.</p>
        </div>
      </div>
      ...
      <ActiveDocumentsTable
        ...
        basePath={`/countries/${country.slug}/documents/outlets`}
        canWrite={canWrite}
        emptyTitle="No outlet-scoped documents"
        emptyDescription="Documents required separately per outlet will show up here."
      />
    </div>
  )
}
