...
import { UpdateRoleForm } from "@/components/identity/UpdateRoleForm"
import { UpdateScopesForm } from "@/components/identity/UpdateScopesForm"
import { AvailabilityBadge } from "@/components/identity/AvailabilityBadge"
import { SetAvailabilityForm } from "@/components/identity/SetAvailabilityForm"
...
export default async function UserDetailPage({ params }: Props) {
  ...
  if (user.status === "pending" || user.status === "invited") {
    redirect(`/identity/manage/${id}/review`)
  }
  ...
  const canEditRole   = session.permissions.includes(AdminPermissions.ADMIN_USERS_ROLES_ASSIGN)
  const canEditScope  = session.permissions.includes(AdminPermissions.ADMIN_USERS_PERMISSIONS_MANAGE)
  ...
  const canManageAvailability = session.permissions.includes(AdminPermissions.ADMIN_USERS_ACCOUNTS_MANAGE_AVAILABILITY)
  const currentKeys   = user.permissions?.map((p: any) => p.permission.key) ?? []

  const roles = canEditRole
    ? await adminFetch<AdminRole[]>("/admin/v1/users/meta/roles", { next: { revalidate: 3600 } })
    : []
    ...
    <div className="page-content animate-slide-up">
      ...
      <div className="admin-card flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              {(user.status === "active" || user.status === "suspended") && (
                <AvailabilityBadge availability={user.reviewAvailability} unavailableUntil={user.unavailableUntil} />
              )}
              ...
              {canEditRole && (
                <UpdateRoleForm
                  ...
                  currentRoleName={user.role?.displayName ?? "—"}
                />
              )}
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {canManageAvailability && (user.status === "active" || user.status === "suspended") && (
            <SetAvailabilityForm
              userId={id}
              availability={user.reviewAvailability}
        ...
        <div className="admin-card space-y-3">
          <div className="flex items-center justify-between">
            ...
            {canEditScope && (
              <UpdateScopesForm
                userId={id}
                currentScopes={user.scopes as any[]}
                isGlobalActor={session.scope.isGlobal}
                actorCountries={session.scope.countryIds}
              />
            )}
          </div>
          {user.scopes.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">No scope assigned.</p>
          )}
        </div>
      )}
