...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `Vendor-scoped documents — ${countrySlug}` }
}

export default async function VendorScopedDocumentsPage({ params, searchParams }: Props) {
  ...
  const result = await adminFetch<DocumentTypeListResult>(
    `/admin/v1/document-types?countryId=${country.id}&status=ACTIVE&scope=VENDOR&page=${page}&pageSize=${PAGE_SIZE}`,
  ...
  return (
    <div className="page-content animate-slide-up">
      ...
      <div className="admin-card flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight text-foreground">Vendor-Scoped Documents</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">Required once per vendor account in {country.name}.</p>
        </div>
      </div>
      ...
      <ActiveDocumentsTable
        ...
        basePath={`/countries/${country.slug}/documents/vendors`}
        canWrite={canWrite}
        emptyTitle="No vendor-scoped documents"
        emptyDescription="Documents required once per vendor account will show up here."
      />
    </div>
  )
}
