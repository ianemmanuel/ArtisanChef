...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `Banned vendors — ${countrySlug}` }
}

export default async function CountryBannedVendorsPage({ params, searchParams }: Props) {
  ...
  const qs = new URLSearchParams({ page, pageSize: String(PAGE_SIZE), countrySlug: country.slug, bannedOnly: "true", sort, dir })
  ...
  return (
    <div className="page-content animate-slide-up">
      ...
      <div className="admin-card flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight text-foreground">Banned Vendors</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">Permanently blocked in {country.name}. Unban from the vendor's own page.</p>
        </div>
      </div>
      ...
      <CountryVendorsTable
        ...
        sort={sort}
        dir={dir}
        basePath={`/countries/${country.slug}/vendors/banned`}
        emptyTitle="No banned vendors"
        emptyDescription="Vendors banned in this country will show up here."
      />
    </div>
  )
}
